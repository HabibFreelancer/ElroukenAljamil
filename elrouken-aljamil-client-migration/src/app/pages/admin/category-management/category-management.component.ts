...
export class CategoryManagementComponent implements OnInit {
  ...
  constructor(private menuAdminService: MenuAdminService, private categoryAdminService: CategoryAdminService) {}
  ...
  loadMenus() {
    this.menuAdminService.getAll().subscribe(data => this.menus = data);
  }

  loadCategories() {
    this.categoryAdminService.getAll().subscribe(data => {
  ...
  save() {
    ...
    const op = this.editingCategory
      ? this.categoryAdminService.update(payload.id, payload)
      : this.categoryAdminService.create(payload);
    op.subscribe(() => { this.loadCategories(); this.cancel(); });
  }
  ...
  delete(id: number) {
    if (confirm('Supprimer cette catégorie ?')) {
      this.categoryAdminService.delete(id).subscribe(() => this.loadCategories());
    }
  }
