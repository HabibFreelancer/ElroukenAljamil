...
export class DeposerAnnonceComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private annonceService: AnnonceService, private workflowService: WorkflowService, private authService: AuthService, private router: Router) {}
    ...
    this.searchSubject.pipe(debounceTime(300)).subscribe(query => {
      if (query.length >= 2) {
        if (this.searchCache[query]) {
          ...
          this.annonceService.suggestCategories(query)
            .subscribe(data => {
  ...
  openCategoryBrowser() {
    ...
    if (this.menus.length === 0) {
      this.annonceService.getMenus().subscribe(data => this.menus = data);
    }
  }

  selectMenu(menu: any) {
    this.selectedMenuId = menu.id;
    this.annonceService.getCategoriesForDeposit(menu.id)
      .subscribe(data => this.browserCategories = data);
  }
  ...
  private checkAdType(categoryName: string, menuName?: string) {
    if (this.selectedCategoryId) {
      this.annonceService.getAdTypes(this.selectedCategoryId)
        .subscribe(data => {
          this.adTypes = data;
  ...
  validateImmatriculation() {
    ...
    this.annonceService.lookupVehicle(immat).subscribe({
      next: (data) => {
  ...
  generateDescription(fieldKey: string) {
    ...
    this.annonceService.generateDescription(context).subscribe({
      next: (res) => {
  ...
  loadPriceEstimate() {
    ...
    this.annonceService.getPriceEstimate(
  ...
  saveDraft() {
    ...
    this.annonceService.saveDraft(payload).subscribe({
      next: () => {
  ...
  submit() {
    ...
    this.annonceService.submit(payload).subscribe({
      next: (res) => {
  ...
  finishFeedback() {
    this.annonceService.submitFeedback(this.submittedAnnonceId!, this.feedbackRating, this.annonce.category).subscribe();
