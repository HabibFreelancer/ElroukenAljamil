...
searchText = '';
  ...
  constructor(private workflowAdminService: WorkflowAdminService, private menuAdminService: MenuAdminService, private annonceService: AnnonceService) {}
  ...
  // === Data loading ===
  loadWorkflows() {
    this.workflowAdminService.getAll().subscribe(data => {
      this.workflows = data;
      this.applyFilter();
    });
  }
  ...
  loadCategories() {
    this.menuAdminService.getAll().subscribe(menus => {
      this.categories = [];
      menus.forEach(m => {
  ...
  saveWorkflow() {
    ...
    const op = this.editingWorkflow
      ? this.workflowAdminService.update(this.currentWorkflow.id, payload)
      : this.workflowAdminService.create(payload);
    op.subscribe(() => { this.loadWorkflows(); this.cancelWorkflow(); });
  }
  ...
  toggleWorkflowActive(wf: any) {
    this.workflowAdminService.update(wf.id, { ...wf, isActive: !wf.isActive }).subscribe(() => this.loadWorkflows());
  }

  deleteWorkflow(id: number) {
    if (confirm('Supprimer ce workflow et toutes ses étapes/champs ?')) {
      this.workflowAdminService.delete(id).subscribe(() => {
        this.loadWorkflows();
  ...
  selectWorkflow(wf: any) {
    ...
    this.selectedStep = null;
    this.fields = [];
    this.workflowAdminService.getSteps(wf.id).subscribe(data => this.steps = data);
  }

  duplicateWorkflow(wf: any) {
    this.workflowAdminService.create({ categoryId: wf.categoryId, name: wf.name + ' (copie)', description: wf.description, isActive: false }).subscribe(() => this.loadWorkflows());
  }

  // === Step CRUD ===
  saveStep() {
    const op = this.editingStep
      ? this.workflowAdminService.updateStep(this.currentStep.id, this.currentStep)
      : this.workflowAdminService.createStep(this.selectedWorkflow.id, this.currentStep);
    op.subscribe(() => { this.refreshSteps(); this.cancelStep(); });
  }
  ...
  toggleStepActive(step: any) {
    this.workflowAdminService.updateStep(step.id, { ...step, isActive: !step.isActive }).subscribe(() => this.refreshSteps());
  }

  deleteStep(id: number) {
    if (confirm('Supprimer cette étape et tous ses champs ?')) {
      this.workflowAdminService.deleteStep(id).subscribe(() => {
        this.refreshSteps();
  ...
  selectStep(step: any) {
    ...
    this.workflowAdminService.getFields(step.id).subscribe(data => this.fields = data);
  }

  refreshSteps() {
    this.workflowAdminService.getSteps(this.selectedWorkflow.id).subscribe(data => this.steps = data);
  }

  // === Field CRUD ===
  saveField() {
    const op = this.editingField
      ? this.workflowAdminService.updateField(this.currentField.id, this.currentField)
      : this.workflowAdminService.createField(this.selectedStep.id, this.currentField);
    op.subscribe(() => { this.refreshFields(); this.cancelField(); });
  }
  ...
  toggleFieldActive(field: any) {
    this.workflowAdminService.updateField(field.id, { ...field, isActive: !field.isActive }).subscribe(() => this.refreshFields());
  }

  deleteField(id: number) {
    if (confirm('Supprimer ce champ ?')) {
      this.workflowAdminService.deleteField(id).subscribe(() => this.refreshFields());
    }
  }
  ...
  refreshFields() {
    this.workflowAdminService.getFields(this.selectedStep.id).subscribe(data => this.fields = data);
  }
